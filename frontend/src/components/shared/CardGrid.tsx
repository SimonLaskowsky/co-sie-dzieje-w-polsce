...
const CardGrid = ({ searchQuery, selectedTypes, data }: CardGridProps) => {
  ...
  const openModal = (card: Act) => {
    if (
      !canOpen ||
      ((user?.unsafeMetadata?.clicks_this_month as number) >= 5 &&
        user?.unsafeMetadata?.subscription_status !== 'active')
    )
      return;
  ...
  return (
    <div className="w-full max-w-screen-xl mx-auto px-2.5">
      {/* TODO: jakoś postylować i rozkminić jak to ładnie wyświetlić */}
      <SignedOut>
        <div
