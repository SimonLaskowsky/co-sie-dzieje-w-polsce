...
def get_new_acts(
    items: List[Dict[str, Any]], last_known: Dict[str, Any]
) -> List[Dict[str, Any]]:
    new_acts = []
    for act in items:
...
def fetch_and_filter_acts() -> List[Dict[str, Any]]:
    ...
    filtered_items = [
        item for item in items if item.get("type") in ["Ustawa", "Rozporządzenie"]
    ]
...
def identify_new_acts(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if not last_known:
        print(
            "📄 No previously saved act found — processing all available legal acts..."
        )
        return items
        ...
        if check_voting_details(eli):
            ...
            if act_details:
                ...
                if pdf_text:
                    ...
                    print(
                        f"❌ Failed to fetch PDF text for act: {act_details.get('title')}"
                    )
