...
def analyze_text_with_openai(text, prompt, max_tokens=1000):
    logger.info(f"Przetwarzanie tekstu, długość: {len(text)} znaków")
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Niepoprawny JSON: {content}")
            return {"error": "Niepoprawny format odpowiedzi", "raw_content": content}
...
def split_and_analyze_text(text, chunk_size=3000, chunk_overlap=200):
    ...
    for i, chunk in enumerate(chunks):
        logger.info(f"Streszczenie fragmentu {i+1}/{len(chunks)}")
        summary = summarize_fragment(chunk)
        summaries.append(summary)

    combined_summary = "\n".join(summaries)
    logger.info(f"Połączono streszczenia, długość: {len(combined_summary)} znaków")
...
def save_analysis_to_file(analysis, filename):
    try:
        ...
        logger.info(f"Zapisano analizę do {filename}")
