...
const DialogModal = ({ isOpen, onClose, card }: DialogModalProps) => {
  ...
  const pieChartData = [
    { name: "Rządzący", value: card.governmentPercentage, fill: chartConfig.government.color },
    { name: "Opozycja", value: 100 - card.governmentPercentage, fill: chartConfig.opposition.color },
  ];

  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
     
      <DialogContent className='overflow-auto w-4/5 h-4/5 !max-w-[1000px] !max-h-[800px] rounded-3xl flex flex-col gap-6'> {/* Te style napisują shadcn temu tak dziwnie */}
        <DialogHeader className='h-fit'>
          <DialogTitle className='text-2xl font-bold leading-tight tracking-tighter'>{card.title}</DialogTitle>
          <DialogDescription>
        ...
        {card.categories && card.categories.length > 0 && (
            <div className="flex flex-col space-y-1.5">
                <div className="font-semibold tracking-tight text-xl">Kategorie uchwały</div>
