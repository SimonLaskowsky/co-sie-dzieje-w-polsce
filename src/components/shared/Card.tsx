...
const Card = ({
  ...
  return (
    ...
    >
      <h3 className="text-lg leading-snug font-semibold tracking-tight line-clamp-3">{title}</h3>
      {categories.length > 0 && (
      ...
      <div className="flex flex-col items-center mt-0.5 gap-1">
        <div className="flex justify-between w-full">
          {[...Array(governmentDots)].map((_, index) => (
            <div key={`gov-${index}`} className="w-2 h-2 bg-neutral-100 rounded-full"></div>
          ))}
          {[...Array(oppositionDots)].map((_, index) => (
            <div key={`opp-${index}`} className="w-2 h-2 bg-red-500/50 rounded-full"></div>
          ))}
        </div>
